import { useEffect, useMemo, useState } from "react";
import { FaCopy, FaWhatsapp } from "react-icons/fa";
import { Modal, Button } from "../Ui";
import { buildWhatsAppMessage, formatMobileNumber } from "../../utils/leadContact";

const copy = (text) => {
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).catch(() => {});
  } else {
    const el = document.createElement("textarea");
    el.value = text;
    document.body.appendChild(el);
    el.select();
    document.execCommand("copy");
    document.body.removeChild(el);
  }
};

/**
 * Modal that shows the exact formatted WhatsApp message before sending.
 *
 * Props:
 *  - open, onClose
 *  - lead: { businessName, mobileNumber, summary, demoLink, websiteLink }
 *  - message: optional pre-built message (used when re-sending a stored message)
 *  - onEdit: opens the edit form (Create Lead page)
 *  - onSend: async () => void (called on "Send Now")
 *  - sending: boolean
 *  - title: override heading
 */
export default function WhatsAppPreviewModal({ open, onClose, lead, message: presetMessage, onEdit, onSend, sending = false, title = "Preview WhatsApp message" }) {
  const [copied, setCopied] = useState(false);

  const message = useMemo(() => {
    if (presetMessage) return presetMessage;
    return lead ? buildWhatsAppMessage(lead) : "";
  }, [presetMessage, lead]);

  useEffect(() => {
    if (open) setCopied(false);
  }, [open]);

  if (!open || !lead) return null;

  const handleCopy = () => {
    copy(message);
    setCopied(true);
    setTimeout(() => setCopied(false), 1600);
  };

  return (
    <Modal
      open={open}
      onClose={onClose}
      title={title}
      size="md"
      footer={
        <>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          {onEdit && <Button variant="secondary" onClick={onEdit}>Edit</Button>}
          <Button onClick={onSend} loading={sending} className="bg-[#25D366] hover:bg-[#1fb959]">
            <FaWhatsapp /> Send Now
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-base/80 px-4 py-3 text-sm">
          <div>
            <span className="text-ink/50">Sending to </span>
            <span className="font-semibold text-ink">{lead.businessName}</span>
            <span className="ml-2 text-ink/60">{formatMobileNumber(lead.mobileNumber)}</span>
          </div>
          <button
            onClick={handleCopy}
            className="inline-flex items-center gap-2 rounded-lg border border-ink/15 px-3 py-1.5 text-xs font-semibold text-ink/70 transition-colors hover:border-primary/40 hover:text-primary"
          >
            <FaCopy /> {copied ? "Copied!" : "Copy message"}
          </button>
        </div>

        <div className="rounded-2xl bg-[#E7F8EC] p-4 sm:p-5">
          <div className="mx-auto max-w-md rounded-2xl rounded-tr-sm bg-white p-4 shadow-card">
            <div className="whitespace-pre-wrap text-[13.5px] leading-relaxed text-ink/85">{message}</div>
            <div className="mt-3 flex items-center justify-end gap-1.5 text-[10px] text-ink/35">
              <span>WhatsApp</span>
              <FaWhatsapp className="h-3 w-3 text-[#25D366]" />
            </div>
          </div>
        </div>

        <p className="text-center text-xs text-ink/45">
          When Cloud API credentials are configured the message is sent directly; otherwise WhatsApp Web will open in a new tab.
        </p>
      </div>
    </Modal>
  );
}
