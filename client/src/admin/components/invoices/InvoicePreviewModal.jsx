...
import { FaCopy, FaFilePdf, FaWhatsapp, FaEnvelope, FaFileInvoice } from "react-icons/fa6";
import { Modal, Button } from "../Ui";
import { buildInvoiceMessagePreview, formatMobileNumber, formatDate } from "../../utils/invoice";
...
/**
 * Modal that previews what will be sent for an invoice: the WhatsApp message,
 * the email body (when channel is email/both) and the PDF attachment.
 * Props: open, onClose, invoice, channel, sending, onSend, title
 */
export default function InvoicePreviewModal({
  open, onClose, invoice, channel = "whatsapp", sending = false, onSend, title = "Preview invoice message",
}) {
  const [copied, setCopied] = useState(false);

  const message = useMemo(() => (invoice ? buildInvoiceMessagePreview(invoice, { includePdfLink: channel === "whatsapp" }) : ""), [invoice, channel]);
  const subject = useMemo(() => (invoice?.invoiceNumber ? `Invoice ${invoice.invoiceNumber} from Skyntrix Technologies` : "Invoice from Skyntrix Technologies"), [invoice]);
      ...
      footer={
        <>
          ...
          <Button onClick={onSend} loading={sending} className={channel === "email" ? "" : "bg-[#25D366] hover:bg-[#1fb959]"}>
            {channel === "email" ? <FaEnvelope /> : <FaWhatsapp />} Send Invoice
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-base/80 px-4 py-3 text-sm">
          <div>
            ...
            {channel !== "email" && <span className="ml-2 text-ink/60">{formatMobileNumber(invoice.mobile)}</span>}
            {channel !== "whatsapp" && invoice.email && <span className="ml-2 text-ink/60">&lt;{invoice.email}&gt;</span>}
          </div>
          {channel !== "email" && (
            <button
              onClick={handleCopy}
        ...
        <div className="flex items-center gap-3 rounded-xl border border-base bg-base/40 px-4 py-3 text-sm text-ink/70">
          <FaFileInvoice className="h-5 w-5 shrink-0 text-red-500" />
          <span>
            The invoice PDF is attached first as a document{" "}
            <span className="font-semibold">{"📄 "}{invoice.invoiceNumber || "Invoice"}.pdf</span>
            {invoice.dueDate && <> · due {formatDate(invoice.dueDate)}</>}
            {invoice.pdfUrl && (
              <a href={`${invoice.pdfUrl}?v=${encodeURIComponent(Date.now())}`} target="_blank" rel="noreferrer" className="ml-1 break-all font-medium text-primary hover:underline">(preview)</a>
            )}
          </span>
        </div>

        <p className="text-center text-xs text-ink/45">
          Without Cloud API credentials, WhatsApp Web opens in a new tab with the invoice message and you send the PDF
